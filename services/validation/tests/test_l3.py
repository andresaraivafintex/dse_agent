"""WSE-E4-T9b — full L3: reflection in the tracking comment (<1min, via the
foundation's MutableCommentWriter), targeted re-runs on fix commits (the real
GitHub API stood in for by FakeGitHubClient — same interface as RealGitHubClient,
which really implements POST .../rerequest) and CI-repair skill-learning episodes
(tenant-scoped, with provenance; NO skill created/activated).
Real Postgres for state/evidence/audit."""
...
def test_reflection_updates_single_tracking_comment_under_1min(work_item_id, tenant_id):
    ...
    assert elapsed < 60, "reflection must happen in the same call (<1min)"
    assert result.status == "red"

    assert len(client._comments) == 1  # EXACTLY 1 comment
    body = next(iter(client._comments.values()))
    assert "`red`" in body and "| tests |" in body and "failure" in body

    # the second call EDITS the same comment (in-place, never comment-per-update)
...
def test_targeted_rerun_only_failed_checks_on_fix_commit(work_item_id, tenant_id):
    client = FakeGitHubClient()
    writer = _writer(client)
    # 1st poll: red on sha-a (checks 2 and 3 failed)
    client.set_check_runs(REPO, "sha-a", _runs_red())
    consume_ci_status_l3(
        github_client=client, work_item_id=work_item_id, tenant_id=tenant_id,
        repo=REPO, pr_number=42, ref="sha-a", comment_writer=writer, surface_ref=SURFACE,
    )
    # fix commit sha-b arrives with the SAME checks still red (CI reused the result)
    ...
    # re-run requested ONLY for the fix commit's failed check-runs (22 and 23; never 21)
...
def test_rerun_gracefully_skipped_when_ci_does_not_support_it(work_item_id, tenant_id):
    client = FakeGitHubClient()
    client.rerequest_supported = False  # repo CI without per-job re-run (403/422)
    writer = _writer(client)
    client.set_check_runs(REPO, "sha-a", _runs_red())
    consume_ci_status_l3(
        github_client=client, work_item_id=work_item_id, tenant_id=tenant_id,
    ...
    assert result.status == "red"  # carries on without re-run, never blows up/blocks
...
def test_repair_episode_emitted_on_red_to_green_with_provenance(work_item_id, tenant_id):
    ...
    assert len(episodes) == 2  # tests + build (the 2 repaired checks)
    ...
    # NO skill created/activated — only the episode (promotion is Phase 4).
    conn = db.get_connection()
    try:
        with conn.cursor() as cur:
...
def test_repeated_pattern_increments_occurrence_tenant_scoped(work_item_id, tenant_id):
    """The SAME failure pattern repaired in another work item of the SAME tenant
    increments occurrence_n; another tenant starts back at 1 (tenant-scoped)."""
    ...
    assert second["occurrence_n"] == 2  # REPEATED pattern detected
    assert other_tenant["occurrence_n"] == 1  # never crosses tenants
